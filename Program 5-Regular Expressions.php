...
echo "<p>Letters Found: " . htmlspecialchars(implode(",", $matches[0])) . "</p>";
$modifiedText = preg_replace('/\s/', '_', $userInput);
echo "<p>Spaces replaced with underscores: " . htmlspecialchars($modifiedText) . "</p>";
$splitText = preg_split('/,/', $userInput, -1, PREG_SPLIT_NO_EMPTY);
echo "<p>Split input using commas: " . htmlspecialchars(implode(",", $splitText)) . "</p>";
}
?>
</body>
</html>
